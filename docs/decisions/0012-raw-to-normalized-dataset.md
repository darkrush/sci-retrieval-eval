...
4. `normalized_dataset` manifest 必须显式写入对 `raw_dataset` 的 dependency。
5. `normalized_dataset` metadata 至少记录：
   - `source`
   - `task_name`
   - `split`
   - `normalizer_name`
...
6. raw JSONL 解析必须按流式逐行读取，不能通过 `body.read().decode()` 一次性读取大文件。

## Consequences

1. `normalized_dataset` 现在有了明确的 raw 上游身份。
2. 标准化逻辑与 raw source 打开方式解耦，测试可以用 fake opener，生产可后续接 S3 opener。

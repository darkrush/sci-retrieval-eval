...
from eval_platform.chunking import CHUNKED_CORPUS_ARTIFACT_TYPE, read_chunked_corpus_artifact
...
from eval_platform.embeddings.artifact import EmbeddingShard, write_embeddings_artifact
...
def run_embedding(
    ...
    *,
    ...
    chunk_shards = iter_chunk_shards(source_store, config.source_artifact_id)
    chunked_corpus = read_chunked_corpus_artifact(source_store, config.source_artifact_id)
    batch_size = config.batch_size or max(1, len(chunked_corpus.chunks))
    records: list[EmbeddingRecord] = []
    embedding_shards: list[EmbeddingShard] = []

    for shard_index, chunk_shard in enumerate(chunk_shards, start=1):
        shard_records: list[EmbeddingRecord] = []
        processed_in_shard = 0
        shard_texts = [chunk.text for chunk in chunk_shard.chunks]
        shard_batches = _batched_texts(shard_texts, batch_size) if shard_texts else []
        cursor = 0
